...
lines.each_with_index.map { [_1.chomp.chars.each_with_index.to_a, _2] }.each do |row, y|
  row.each { |char, x| table[char] += [Complex(x, y)] unless char == '.' }
end

ANTINODES = Set.new

def add_node(node)
  ANTINODES << node if (0...WIDTH).cover?(node.real) && (0...HEIGHT).cover?(node.imag)
end

table.each_value do |positions|
  positions.combination(2).each do |a, b|
    diff = b - a
    (0..).find { !add_node(b + _1 * diff) }
    (0..).find { !add_node(a - _1 * diff) }
  end
end
