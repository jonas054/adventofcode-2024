...
def main(
  ...
  val bytes: Array[Int] = input.map { (pair: String) =>
    val xy = pair.split(',')
...
def findShortestPath(roads: Set[Int], max: Int): Int = {
  val graph = Map[Int, Map[Int, Int]]()
  roads.foreach(node =>
    graph(node) = Map[Int, Int]()
    DIRECTIONS.foreach { dir =>
      if (roads.contains(node + dir))
        graph(node)(node + dir) = 1
    }
  )
  return dijkstra(graph, Point(0, 0))(Point(max, max))
}
...
def dijkstra(graph: Map[Int, Map[Int, Int]], start: Int): Map[Int, Int] = {
  ...
  while (nodes.size > 0) {
    ...
    graph(minNode).foreach { (neighbor, value) =>
      distances(neighbor) = (distances(minNode) + value).min(distances(neighbor))
    }
    visited.add(minNode)
