...
def costs_for_successful(ax, bx, prize_x, ay, by, prize_y)
  t = ax * prize_y - ay * prize_x
  n = ax * by - ay * bx
  return 0 if t % n != 0

  b = t / n
  return 0 if (prize_x - bx * b) % ax != 0

  a = (prize_x - bx * b) / ax
  3 * a + b
end

def main(input, offset)
  ax = ay = bx = by = prize_x = prize_y = nil
  costs = []
  input.lines.each do |line|
    ...
    when /Button B: X\+(\d+), Y\+(\d+)/
      bx, by = $1.to_i, $2.to_i
    when /Prize: X=(\d+), Y=(\d+)/
      prize_x = offset + "#$1".to_i
      prize_y = offset + "#$2".to_i
      costs << costs_for_successful(ax, bx, prize_x, ay, by, prize_y)
    end
  end
...
print 'part 1, example: ', main(EXAMPLE, 0), "\n" # 480
print 'part 1, input: ', main(PUZZLE_INPUT, 0), "\n" # 35574
print 'part 2, example: ', main(EXAMPLE, 10_000_000_000_000), "\n" # 875318608908 ?
print 'part 2, input: ', main(PUZZLE_INPUT, 10_000_000_000_000), "\n" # 80882098756071

__END__

a: number of presses on A
b: number of presses on B
ax: how much A moves the X axis
bx: how much B moves the X axis
ay: how much A moves the Y axis
by: how much B moves the Y axis
prize_x, prize_y: the position of the prize

ax * a + bx * b == prize_x
ay * a + by * b == prize_y

# Extract a and b
a == prize_x / ax - bx * b / ax
b == prize_y / by - ay * a / by

# Remove a from the equation and calulate b
b == prize_y / by - ay * (prize_x / ax - bx * b / ax) / by
by * b == prize_y - ay * prize_x / ax + ay * bx * b / ax
(ax * by - ay * bx) * b == ax * prize_y - ay * prize_x
b == (ax * prize_y - ay * prize_x) / (ax * by - ay * bx)

# Remove b from the equation and calulate a
a == prize_x / ax - bx * (prize_y / by - ay * a / by) / ax
ax * a == prize_x - bx * prize_y / by + bx * ay * a / by
ax * by * a == by * prize_x - bx * prize_y + bx * ay * a
ax * by * a - bx * ay * a == by * prize_x - bx * prize_y
a == (by * prize_x - bx * prize_y) / (ax * by - ay * bx)
