...
long costForSuccessful(int ax, int bx, long prizeX, int ay, int by, long prizeY) {
  long t = ax * prizeY - ay * prizeX;
  int n = ax * by - ay * bx;
  if (t % n != 0)
    return 0;
  ...
  long a = (prizeX - bx * b) / ax;
  return 3 * a + b;
}

bool startsWith(std::string whole, std::string start) {
  return whole.substr(0, start.length()) == start;
}

long run(const char** input, long offset) {
  int ax = 0, ay = 0, bx = 0, by = 0;
  long totalCost = 0;
  for (int ix = 0; input[ix]; ++ix) {
    const char* line = input[ix];
    if (startsWith(line, "Button A"))
      sscanf(line, "Button A: X+%d, Y+%d", &ax, &ay);
    else if (startsWith(line, "Button B"))
      sscanf(line, "Button B: X+%d, Y+%d", &bx, &by);
    else if (startsWith(line, "Prize")) {
      int prizeX, prizeY;
      sscanf(line, "Prize: X=%d, Y=%d", &prizeX, &prizeY);
      totalCost += costForSuccessful(ax, bx, offset + prizeX, ay, by, offset + prizeY);
    }
  }
...
int main() {
  cout << "part 1, example: " << run(EXAMPLE, 0) << "\n"; // 480
  cout << "part 1, input: " << run(PUZZLE_INPUT, 0) << "\n"; // 35574
  cout << "part 2, example: " << run(EXAMPLE, 10000000000000) << "\n"; // 875318608908 ?
  cout << "part 2, input: " << run(PUZZLE_INPUT, 10000000000000) << "\n"; // 80882098756071
  return 0;
}
